...
所有 preset 共享的固定门槛 (不在这里写, 在 GatesConfig 的默认值里):
  For parts 排除、卖家最低线 98% / 100 条、warranty ≥1 年 (TOLERANT, 缺失放行)

件况完全不进 gate: "偏新" 是软偏好, 由质量分的件况子分体现
(New=100 > New other=90 > Reman=82 > Used=75 > Acceptable=60)。
四档现在 gate 完全相同 (都只有 in_stock), 差别全在三维权重上。
...
RUSH = Preset(
    ...
    gates=GatesConfig(require_in_stock=True),
    scoring=ScoringConfig(),
...
# 省钱: 不急, 越便宜越好 —— 价格权重压倒性 (60), 但仍要求在库
BUDGET = Preset(
    name="Budget",
    description="不急, 越便宜越好",
    gates=GatesConfig(require_in_stock=True),
    scoring=ScoringConfig(),
    weights=Weights(price=60, speed=10, quality=30),
)

# 优质: 高端车 / 严苛保险 —— 质量权重 60, 不硬排除非新件
# 卖家门槛也不单独抬 (靠质量分权重区分, 不靠 gate 偏好大卖家)
PREMIUM = Preset(
    name="Premium",
    description="高端车 / 严苛保险, 质量优先",
    gates=GatesConfig(require_in_stock=True),
    scoring=ScoringConfig(),
